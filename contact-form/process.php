...
// if the language has been defined in a hiden field, use that instead of the JSON default language
$lang = (!empty($fdata['lang'])) ? $fdata['lang'] : $lang;
...
function getValidationErrors()
{
	...
	foreach ( $fdata as $field => $value ) {
		...
		switch ($field) {
			...
			// security token
			case 'ucf_csrf_token':
				if ($_SESSION['ucf_csrf_token'] != $fdata['ucf_csrf_token']) {
					array_push($errors, $translations->form->error->script->$lang);
				}
				break;
		}
	}
	...
	if ( isset($_POST['g-recaptcha-response'])  ) {
		...
		// otherwise it is set, and we need to verify it
			...
			$fdata['captcha_score'] = '<a href="https://developers.google.com/recaptcha/docs/v3#score">'.$response->score.'</a>';

			// if we have response data
      if(!empty($response)){

				// reject any failed responses
        if(
					$response->success === false || 
					$response->score < $options['captcha_score_threshold']
				){
...
function checkIfDataHasBeenSent()
{
	...
	if(!isset($_SESSION['ucf_csrf_token'])){
		if ($is_ajax) {
			$error = array('errors' => array($translations->form->error->direct_access->$lang));
			echo json_encode($error);
		} else {
			echo '<h1>'.$translations->form->error->title->$lang.'</h1>';
			echo '<p>'.$translations->form->error->direct_access->$lang.'</p>';
		}
		exit;
	}
...
function getHtmlEmailContent()
{
  ...
  if($options['send_user_ip']){
    $content .= '<tr>';
    $content .=   '<th>IP</th>';
		$content .=   '<td><a href="https://www.ip-tracker.org/locator/ip-lookup.php?ip='.$_SERVER['REMOTE_ADDR'].'">'.$_SERVER['REMOTE_ADDR'].'</a></td>';
    $content .= '</tr>';
  }

  foreach ($fdata as $field => $value) {
    if($field !== 'g-recaptcha-response' && $field !== 'ucf_csrf_token'){
      if (empty($value)) {
        $value = '&mdash;';
      }
      $content .= '<tr>';
...
function getPlainEmailContent()
{
  ...
  if($options['send_user_ip']){
    $content .= 'IP: '.$_SERVER['REMOTE_ADDR'].PHP_EOL;
  }
...
function emailSuccessfullySent()
{
  ...
  if(!$options['attachment_store']) { removeUploadsFromServer(); }
