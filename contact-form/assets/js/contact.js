...
jQuery.getJSON( 'contact-form/translations.json', function(json) {
	translations = json;
	lang = (jQuery('html')[0].lang) ? jQuery('html')[0].lang : json.default_lang;
	window.console.log('translations file:',json);
 ...
 */
function resetFormButton ($form, $submit, submit_text) {
	jQuery('<button class="button secondary">'+translations.form.reset[lang]+'</button>')
		.insertAfter(jQuery($form.find('.button.primary')))
		.click(function(e){
			e.preventDefault();
			$reset = jQuery(this);
			...
			// reset form fields
			$form.find('.field').val('');
			$form.find('select.field').select2('val', '');
...
function recaptchaReset () {
	var recaptcha_script = jQuery('script[src^="https://www.google.com/recaptcha/api.js?render="]');
	if(recaptcha_script.length){
		window.console.log('resetting google recaptcha response');
		var recaptcha_key = recaptcha_script.attr('src').replace('https://www.google.com/recaptcha/api.js?render=', '');
		grecaptcha.execute(recaptcha_key, {action: 'contactformresetting'}).then(function(token) {
...
jQuery(document).ready(function($) {
	...
	$('form.ucf').each(function(){
		...
		// if the HTML has a lang attribute, add it as a hidden field to pass to PHP
		if(jQuery('html')[0].lang) {
			$form.append('<input type="hidden" name="lang">');
		}
		...
		$form.submit(function(e){
            ...
            recaptchaReset();
					...
					if(data.success) {
						...
						resetFormButton($form, $submit, submit_txt);
					}
